...
class Pipeline(models.Model):
    ...
    def get_processing_config(self, name):
        ...
        if not resp.text:
            raise requests.exceptions.RequestException(_('Pipeline %(pipeline)s: empty processing configuration (%(name)s).') % {'pipeline': self, 'name': name})
        return resp.text
    ...
    def reingest(self, name, uuid, target='transfer'):
        ...
        if resp.status_code != requests.codes.ok:
            try:
                ...
                raise requests.exceptions.RequestException(_('Pipeline %(pipeline)s returned an unexpected status code: %(status_code)s (%(error)s)') % {'pipeline': self, 'status_code': resp.status_code, 'error': json_error})
